...
**บันทึกรูปผลการทดลอง**
```text
บันทึกรูปผลการทดลองที่นี่![alt text](image-2.png)
```
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะ Container ที่กำลังทำงานอยู่ docker ps -a แสดง Container ทั้งหมด ทั้งที่กำลังทำงานและหยุดแล้ว
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > _คำตอบ_: --rm ใช้ในกรณีที่ต้องการให้ Docker ลบ Container อัตโนมัติหลังจากหยุดการทำงาน เพื่อไม่ให้มี Container ค้างอยู่ในระบบ

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it ใช้ สร้างและเริ่ม Container ใหม่ พร้อมเข้าใช้งานแบบ interactive docker exec -it ใช้ เข้าไปทำงานใน Container ที่กำลังรันอยู่แล้ว โดยไม่สร้าง Container ให
...
```
การสร้าง Docker image หลายขั้นตอนภายใน Dockerfile เดียว
โดยใช้ขั้นแรกสำหรับ build / compile โปรแกรม และใช้ขั้นสุดท้ายเฉพาะ ไฟล์ที่จำเป็นสำหรับการรันจริง

ช่วยให้ image มีขนาดเล็ก สะอาด และปลอดภัยมากขึ้น
```
---
...
ใช้กำหนด Environment Variable ให้กับ Container
โดยตั้งค่าตัวแปรชื่อ DB_PATH ให้ชี้ไปยังตำแหน่งไฟล์ฐานข้อมูล
/app/data/booking.db ภายใน Container

-v $(pwd)/test-data:/app/data
ใช้ mount โฟลเดอร์จากเครื่อง Host เข้าไปใน Container (Volume)
โดยนำโฟลเดอร์ test-data จากตำแหน่งปัจจุบันของเครื่อง ($(pwd))
ไปเชื่อมกับโฟลเดอร์ /app/data ภายใน Container
ทำให้ข้อมูลถูกเก็บถาวร แม้ Container ถูกลบข้อมูลยังไม่หาย
```
...
**บันทึกผลการทดลอง**
```
บันทึกรูปผลการรัน curl http://localhost:5000 ![alt text](image-3.png)
```
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: ชื่อ Docker image และเวอร์ชัน
2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: curl http://localhost
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `484` MB | booking-backend:1.0 ขนาด `62.2` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 มักมีขนาดเล็กกว่า เพราะมีเฉพาะไฟล์เว็บที่ build แล้ว (HTML, CSS, JS) และรันผ่าน nginx ส่วน booking-backend:1.0 มี runtime และ dependencies ของ Node.js รวมถึง library ต่าง ๆ ทำให้ image มีขนาดใหญ่กว่า

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ใช้กำหนดให้ nginx รับ request ที่ขึ้นต้นด้วย /api/ แล้วส่งต่อ (proxy) ไปยัง backend server เพื่อให้ frontend สามารถเรียก API ผ่าน nginx ได้

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะ Docker network มีระบบ DNS ภายใน ทำให้ container สามารถเรียกกันด้วยชื่อ container หรือ service name ได้ จึงใช้ชื่อ backend แทน IP address ได้โดยตรง
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml คือไฟล์สำหรับกำหนดและจัดการหลาย Container พร้อมกันในที่เดียว โดยสามารถตั้งค่า image, port, environment, volume และ network ได้ ทำให้รันระบบทั้งหมดด้วยคำสั่งเดียว (docker compose up) สะดวกและลดความผิดพลาดในการตั้งค่า

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็นการตั้งค่าให้ Container เริ่มทำงานใหม่อัตโนมัติเมื่อหยุดหรือเครื่อง restart ยกเว้นกรณีที่ผู้ใช้สั่ง stop ด้วยตนเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume ถูกจัดการโดย Docker และเก็บข้อมูลไว้ภายในระบบ Docker ส่วน Bind Mount คือการเชื่อมโฟลเดอร์จากเครื่อง Host โดยตรง การใช้ Named Volume กับ SQLite ช่วยให้ข้อมูลฐานข้อมูลไม่หายเมื่อ Container ถูกลบ และลดปัญหา permission หรือ path ของเครื่อง Host
...
**บันทึกรูปผลการทดลอง**
```
บันทึกรูปที่นี่![alt text](image-4.png)
```
...
> **📝 บันทึกผลการทดลอง**:
> - IP ของ frontend: 172.18.0.0/16
> - IP ของ backend: 172.18.0.1
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: docker compose ps ใช้สำหรับแสดงสถานะของ Service และ Container ที่รันจาก docker-compose เช่น ชื่อ container, image ที่ใช้, สถานะการทำงาน (running/up), และ port ที่เปิดใช้งาน

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: docker network inspect booking-network \
--format='{{range .Containers}}{{.Name}}: {{.IPv4Address}}{{"\n"}}{{end}}'
